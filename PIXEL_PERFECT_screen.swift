...
private extension PIXEL_PERFECT_screen {
    ...
    func setScreenIsVisible(_ isViewScreen: Bool) {
        ...
        let imageView = baseView.viewWithTag(PPTagView.screenImageView)
        if imageView == nil {
            ...
            NSLayoutConstraint.activate([
                ...
                screenImageView.heightAnchor.constraint(
                    equalToConstant: screenImage.size.height / CGFloat(imageHeightDivider)
                )
            ])
        }
    ...
    @objc func imagePanGestureHandler(_ sender: UIPanGestureRecognizer) {
        ...
        if location.x > superview.bounds.width / 3 {
            changeImageAlpha(superview, sender)
        } else {
            changeImageTopOffset(superview, sender)
        }

        func changeImageAlpha(_ view: UIView, _ sender: UIPanGestureRecognizer) {
        ...
        func changeImageTopOffset(_ view: UIView, _ sender: UIPanGestureRecognizer) {
    ...
    func saveCurrentOffsetMenuActionHandler() {
    ...
    func deleteCurrentOffsetMenuActionHandler() {
    ...
    func getImageTopOffsetUDKey() -> String {
        return "\(imageName)_\(PPKey.imageTopOffsetKeyUD)"
    }
    ...
    func buildSliderCurrentValueUDKey(sliderTitle: String) -> String {
        ...
        return "\(imageName)_\(PPKey.sliderCurrentValueKeyUD)_\(validTitleForKey)"
    }
    ...
    func buildActionsMenu() -> UIMenu {
        ...
        let deleteImageTopOffset = UIAction(
            ...
            image: UIImage(systemName: "pip.remove")
        ) { [weak self] (_) in
            self?.deleteCurrentOffsetMenuActionHandler()
        }
        ...
        let saveImageTopOffset = UIAction(
            ...
            self?.saveCurrentOffsetMenuActionHandler()
        }
